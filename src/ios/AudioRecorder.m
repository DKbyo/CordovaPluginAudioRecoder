...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder
  ...
  NSData *zipFileData = [NSData dataWithContentsOfFile:recorderFilePath];
  
  NSString *base64String = [zipFileData base64EncodedStringWithOptions:0];
  
  [self sendSuccessTo:self.scanCallbackId withObject:base64String];
}

- (void)audioRecorderEncodeErrorDidOccur:(AVAudioRecorder *)recorder
                                   error:(NSError *)error{
     [self sendFailureTo:scanCallbackId];
}
...
- (void)startRecord:(CDVInvokedUrlCommand*)command {
  ...
  scanCallbackId = _command.callbackId;
